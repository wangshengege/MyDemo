...
public class VideoThumbnailLoader {
    ...
    public void display(String url,ImageView iv,int width,int height,ThumbnailListener thumbnailListener){
        ...
        new ThumbnailLoadTask(url,iv,width,height,thumbnailListener).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);//使用AsyncTask自带的线程池

    }
    public void display(LocalVideoEntity entity, ImageView iv){
        iv.setTag(entity.getPath());
        new ThumbnailLoadTask(entity.getPath(), iv, entity.getWidth()==0?300:entity.getWidth(), entity.getHeight()==0?300:entity.getHeight(), new ThumbnailListener() {
            @Override
    ...
    public void display(String url,ImageView iv,int width,int height){
        iv.setTag(url);
        new ThumbnailLoadTask(url, iv, width, height, new ThumbnailListener() {
            @Override
            public void onThumbnailLoadCompleted(String url, ImageView iv, Bitmap bitmap) {
                //通过判断imageview的tag,和我们加载的图片的url是否是同一个来判断是否显示,这样可以避免滑动造成的位置错乱等问题.
                String tag = (String) iv.getTag();
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if (!isEmpty(url)) {
                ...
                if (bitmap == null||bitmap.isRecycled()) {
                    ...
                    File file =new File(FileAccessor.getImagePathName(), getFileName(url)+".png");
                    if (null != file && file.exists()) {//去磁盘缓存取
                        bitmap = BitmapFactory.decodeFile(file.getPath());
                        if (null==bitmap) {
                            bitmap = getVideoThumbnail(url, width, height, MediaStore.Video.Thumbnails.MICRO_KIND);
                            //将图片保存到磁盘文件,作为缓存
                            Utils.saveBitmapToLocal(file.getAbsolutePath(), bitmap);
                        }
                    } else {
                        bitmap = getVideoThumbnail(url, width, height, MediaStore.Video.Thumbnails.MICRO_KIND);
